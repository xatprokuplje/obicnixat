    ...
    */
    async execute (bot, xatID, message, from) {
        if (!bot.state.aiEnabled) {
            // Ne ispisuj ništa u chat kad je AI isključen za ovaj nalog -
            // samo tiho ignoriši komandu (samo log, radi debug-a).
            bot.logger.info(`AiCommand: ignorisano (AI isključen za ovaj nalog).`);
            return;
        }
