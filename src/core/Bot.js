...
export class Bot {
    ...
    async maybeReplyAsAi (xatID, message, from) {
        ...
        // Traži da li poruka pominje NAŠ nadimak kao celu reč (ne kao deo
        // neke druge reči), bez obzira na velika/mala slova - tako bot ne
        // odgovara na SVAKU poruku u chatu, samo kad ga neko direktno
        // oslovi/pomene, baš kao pravi korisnik.
        const escaped = nickname.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const mentionRegex = new RegExp(`(^|\\s)${escaped}([\\s,.!?:;]|$)`, 'i');
        if (!mentionRegex.test(message)) return;
