...
export class Bot {
    ...
    async stop () {
        ...
        if (this.state.ws) {
            ...
            try {
                // Gracefully close (pravi WS close frame ka xat serveru) umesto
                // terminate() - terminate() samo kida TCP konekciju bez ijednog
                // signala serveru, pa xat sesiju smatra "živom" dok mu istekne
                // interni timeout, što izaziva DUP pri sledećem loginu. Sa
                // close() server odmah zna da smo se odjavili. Ako se close ne
                // završi za 2s (npr. konekcija već mrtva), forsiramo terminate
                // kao fallback da ne visimo u nedogled.
    ...
    /**
     * Prirodno AI učešće u chatu - BEZ komandnog prefiksa (!ai). Ako neko
     * u poruci pomene nadimak ovog naloga (kao što bi normalno oslovio
     * nekog u chatu), bot odgovara preko AI-a. Poziva se iz MessageHandler-a
     * za SVAKU poruku koja nije komanda.
     * @param {number|string} xatID - Pošiljalac.
     * @param {string} message - Sadržaj poruke.
     * @param {string} from - Izvor ("main", "pc", "pm").
     */
    async maybeReplyAsAi (xatID, message, from) {
        if (!this.state.aiEnabled) return;

        // Skini power-kodove iz nadimka ((hat3), (glow00ff00)...) pre
        // poređenja - to su xat efekti, ne deo imena kojim nas neko zove.
        const nickname = (this.state.nickname || '').replace(/\([^)]*\)/g, '').trim();
        if (!nickname) return;

        // Prepoznaj i punо ime KAO CELU REČ (npr. "Diaxgalaksija") I
        // svaku reč iz poruke koja je makar 3 slova duga i predstavlja
        // POČETAK našeg imena (case-insensitive) - npr. "dia", "diax",
        // "Dia" sve pogađaju "Diaxgalaksija", bez obzira kako se nadimak
        // trenutno zove. Tako bota može da zove i skraćeno, ne mora puno
        // registrovano ime.
        const nickLower = nickname.toLowerCase();
        const words = message.split(/[^\p{L}\p{N}]+/u).filter(Boolean);
        const mentioned = words.some((w) => {
            if (w.length < 3) return false;
            const wl = w.toLowerCase();
            return nickLower.startsWith(wl) || wl.startsWith(nickLower);
        });
        if (!mentioned) return;

        // Zaštita od bot-bot petlje: ako je pošiljalac neki drugi bot nalog
        // (xat ga vodi kao "bot" tip naloga), ne odgovaramo - inače dva
        // naša naloga mogu beskonačno da se "cackaju" pominjući jedan
        // drugog i troše AI pozive u krug.
        const sender = this.state.getUser(xatID);
        if (sender?.isBot?.()) return;

        // Prost rate-limit po korisniku (15s) - da isti čovek ne može
        // spamovanjem pominjanja imena da potroši sve AI pozive/kredit.
        this._aiMentionCooldown ||= new Map();
        const now = Date.now();
        const last = this._aiMentionCooldown.get(xatID) || 0;
        if (now - last < 15000) return;
        this._aiMentionCooldown.set(xatID, now);

        try {
            const { text, provider, model } = await this.Ai.ask(message);
            this.logger.info(`AI (pomenuto ime) odgovorio preko ${provider}/${model}`);
            await this.reply(text, xatID, from);
        } catch (error) {
            this.logger.error(`AI (pomenuto ime) greška: ${error.message}`);
        }
    }
