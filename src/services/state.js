export class BotState {
    ...
    constructor(accountConfig = {}) {
        ...
        // Samo nalog obeležen kao "primarni" šalje pozdravnu poruku novim
        // korisnicima (inače bi SVAKI povezani bot slao svoju pozdravnu
        // poruku istom korisniku). Podešava se u src/index.js pri paljenju.
        this.isPrimary = accountConfig.isPrimary === true;

        // Lista poruka koje nalog šalje u krug (auto-poruke). Svaka poruka
        // je oblika { text, delaySeconds } - delaySeconds je vreme (u
        // sekundama) čekanja NAKON prethodne poslate poruke ovog naloga,
        // pre slanja baš te poruke. Menja se u letu preko admin panela
        // (vidi Bot.setMessages).
        this.autoMessages = Array.isArray(accountConfig.messages) ? accountConfig.messages : [];
        this.autoMessageIndex = 0;
        this.messageRotationTimeout = null;

        // Da li OVAJ nalog sme da odgovara na !ai komandu. Podešava se u
        // letu preko admin panela (kvadratić pored naloga), bez restarta
        // bota (vidi Bot.setAiEnabled). Podrazumevano uključeno.
        this.aiEnabled = accountConfig.aiEnabled !== false;
        ...
        this.envData = {
            ...
            // Isti Groq/Gemini API ključ se deli između svih naloga u ovom
            // procesu (podešava se jednom u Render env varijablama) - samo
            // se aiEnabled po nalogu odlučuje da li SME da ga koristi.
        ...
        this.cacheDir = dataDir ? `${dataDir}/cache` : './cache';
        this.loginCachePath = `${this.cacheDir}/login_${username}.json`;
    }
